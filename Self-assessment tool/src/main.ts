...
function otherWaysToSignIn(): HTMLElement {
  /**
   * A link waiting to be finished is not one of the other ways in, it is the way in, so the
   * fold is open and the summary says what is under it. Reported as: i only have my work email
   * on my phone. but im only signed in from my work laptop. which i dont have. A person who has
   * just opened their link has to meet the field that finishes it, and a closed fold hides it.
   */
  const finishing = signInLinkNeedsAddress();
  return el('details', { class: 'signin-other', open: finishing || linkEmailWaiting() ? true : undefined }, [
    el('summary', {}, [
      finishing
        ? t('Finish signing in with your link', 'Terminer la connexion avec votre lien')
        : t('Other ways to sign in', 'Autres façons de se connecter'),
    ]),
    emailLinkBlock(),
    /**
...
function emailLinkBlock(): HTMLElement {
   ...
   * not see a note anywhere that there is a daily limit. All three were the same dead end, and
   * it survived a reload, so a link that never arrived left the sign-in screen stuck for good.
   */
  const state = el('div', {});
  ...
  const showSent = (to: string) => {
    ...
    state.appendChild(el('div', { class: 'card tight signin-sent' }, [
      ...
      /**
       * Said because departmental mail filters are the likeliest thing between a link and an
       * inbox: it arrives from noreply at a firebaseapp.com address with no sender name, which
       * is the shape of a message a government filter quarantines.
       */
      el('p', { class: 'tiny dim' }, [
        t('If nothing arrives in a few minutes, look in junk. Mail from an address like this one is often held by a departmental filter, and a personal address is the quickest way to tell which is happening.',
          'Si rien n\u2019arrive en quelques minutes, regardez dans les indésirables. Le courrier provenant d\u2019une adresse comme celle-ci est souvent retenu par un filtre ministériel, et une adresse personnelle est le moyen le plus rapide de savoir ce qui se passe.'),
      ]),
    ]));
  };
...
function paneDocs(pane: HTMLElement) {
  ...
  pane.appendChild(el('div', { class: 'hub-cards' }, [
    ...
    linkCard({
      ...
      href: 'https://docs.google.com/spreadsheets/d/1Hxtjj0EIVlf1hBfQy9hrgR-_U7D3i3t_R38XaHnda9A/edit',
    }),
  ]));
}
