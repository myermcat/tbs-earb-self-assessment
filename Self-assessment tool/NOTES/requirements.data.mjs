...
export const sections = [
  ...
  },
  {
    ...
    reqs: [
      ...
      { id: 'S11', state: 'blocked', owner: 'Mariia, then TBS', text: 'Somebody holding an access code may read and change that one assessment, with no account.',
        note: 'The code is minted, shown, typed and sent to the store, and the store refuses it. The published rule allows get to the owner or an assessor, so today the code only works for somebody who already had access, which makes holding one mean nothing. It needs one rules change, published by whoever owns the project: get and update on an assessment granted on the document id alone. The arithmetic behind it is 12 characters from a 32-symbol alphabet, which is 2^60 and not guessable. What it costs is an open read on a record whose name somebody has, and an unauthenticated script spending the daily read allowance is the risk that goes with it.' },
