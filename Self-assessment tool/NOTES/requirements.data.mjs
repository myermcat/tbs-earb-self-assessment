...
export const sections = [
  ...
  },
  {
    ...
    reqs: [
      ...
      { id: 'S10', state: 'built', text: 'A submitter can name the people working on the assessment with them, and their own assessors. As a mockup: the addresses are recorded and nothing is sent.',
        note: 'One list with a part on each entry, so there is one way to add and one way to remove. Beside it, two flat lists of addresses, derived on every write, because the store\u2019s rule language cannot read a field out of an object inside an array and every record already in the store would otherwise need migrating on the day the real sharing goes live. It says on every screen it touches that no email is sent and no access is granted, the sentence is above the field where it will be read, and nothing on it can open a mail client.' },
      { id: 'S11', state: 'proposed', owner: 'TBS', text: 'A teammate may read and change an assessment they were added to, and nothing else about it.',
        note: 'Needs a rules change, published by whoever owns the project: a teammate predicate reading the flat address list, get and update on the assessment, and the sharing field itself pinned so a teammate cannot add strangers.' },
      { id: 'S12', state: 'proposed', owner: 'TBS', text: 'A named assessor may read an assessment before it is finished, and may not score it until the role is granted.',
        note: 'Naming somebody as your assessor must never grant scoring. The audit is gated on a role document that only an admin writes, and if a named address widened that, anybody could appoint their own assessor.' },
      { id: 'S13', state: 'open', owner: 'Dan', text: 'Whether naming an assessor routes the record to that person.',
        note: 'Every assessor can already list every submitted assessment, so naming one may be a routing hint and not a permission at all. Which of the two it is changes what the assessor screen should show.' },
