...
export const sections = [
  ...
  },
  {
    ...
    reqs: [
      ...
      { id: 'H11', state: 'built', text: 'Signed in means the work is kept at TBS as it is answered. Telling TBS it is ready to review is a separate act.',
        note: 'The write goes three seconds after somebody stops typing, with twenty seconds between writes, which holds one person to 180 an hour against a shared allowance of 20,000 a day. Four guards stand on that write: a build that has a store it can reach, somebody signed in, a marking chosen, and the record owned by the person signed in. The marking guard is the important one: until the person has said how their evidence is marked, nothing leaves the machine. The tab closing and the connection returning both push whatever is queued.' },
