      ...
      { t: 'How an assessor proves who they are', status: 'next',
        ...
        subs: [
          { t: 'A sign-in link sent to a work address', status: 'next', owes: 'Mariia',
            why: 'Built on 23 September and waiting on one console step. It is under Other ways to sign in, below the Google button, which is where every product that offers this puts it. Firebase mails a one-time link, opening it proves the person can read that mailbox, and the address comes back verified, which is what the store\u2019s rules insist on. The step nobody else can do: Firebase console, Authentication, Sign-in method, enable Email/Password and then Email link (passwordless sign-in) under it. Until that is done the screen says so in those words instead of printing the service\u2019s error code. What it costs: five sign-in emails a day for the whole project on the no-cost plan, and 25,000 a day on pay-as-you-go, where the bill for this tool is still nothing, because that plan carries the same free amounts and this tool uses about six per cent of them. Read the five carefully, because it is easy to read as five people. It is five sign-in events a day across the whole project, and a refresh token does not expire, so somebody who signs in once stays signed in until they clear their browser. What it will not survive is a room trying it at the same time, or one person mistyping an address twice: the sixth send that day is refused and there is no way onward. One thing worth knowing for later: the cap of five is Firebase putting mail in the post, and generating a link is a separate limit of 20,000 a day, so a route that generates the link and hands it to another mailer is governed by the larger number. GC Notify is the sanctioned service in the Government of Canada and has a REST interface and no SMTP, which is what makes that a piece of work and not a setting.' },
          { t: 'Microsoft sign-in, and the September note about it was wrong', status: 'next', tier: 'level2',
            why: 'Corrected on 23 September after Mariia pointed out that plenty of applications offer Microsoft sign-in and she uses her school account with them, and nobody asked her school. She is right, and the old note here was the expensive kind of wrong: it turned a free afternoon into a request across an organisational boundary. An application registered once as multi-tenant lets people from any Microsoft directory sign in; the other organisation is never asked in advance, a service principal appears in it the first time somebody from there consents, and registering costs nothing. Microsoft\u2019s default for a new tenant lets a person consent for themselves to the four things a plain sign-in asks for, because the exclusions are files, mail, calendars and the like. So what is actually unknown is narrower and neither part needs TBS. One, whether TBS has switched user consent off, in which case an administrator there has to agree after all. Two, whether Firebase reports a Microsoft address as verified, because the store\u2019s rules refuse any identity that is not, and the reading that it does not is unconfirmed. The client code is already written and the button is on the screen, disabled.' },
