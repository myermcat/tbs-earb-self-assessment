  ...
  },
  {
    ...
    groups: [
      { t: 'How an assessor proves who they are', status: 'next',
        ...
        subs: [
          { t: 'A sign-in link sent to a work address', status: 'next',
            why: 'The route nobody had costed, and the only one that gives Dan his real TBS address without asking anybody outside this team. The rules file already names it as step 3 of setting the store up. It was ruled out in September for a reason that was right then and is wrong here: the free plan sends five sign-in emails a day for the whole project, which is three testers and cannot run a pilot, and is more than enough for one person on one Friday. What it needs is client code that does not exist yet, because the page only knows the Google redirect flow: two more calls, sendOobCode and signInWithEmailLink, and a screen that asks for an address and then waits. Days, not weeks. Nobody outside the team has to agree, the console step is Mariia\u2019s, and it is reversible because the Google button stays where it is. Commitment: low.' },
          { t: 'Microsoft sign-in, which is one approval and a rules decision', status: 'wait', owes: 'IMTD, through Dan', tier: 'level2',
            why: 'The client side is written: signInWithMicrosoft is in src/firebase.ts and goes through the same lines Google uses today, and the button is on the sign-in screen, disabled, with a comment saying it needs an application registered in an Azure directory. Wiring it is an import and two attributes. That is the whole of the cheap part. Two things it is not: the rules refuse any identity without a verified-email claim, and Entra does not assert one, so deploy/firestore.rules is in scope and the clause being changed is the one that stops an unverified address claiming somebody else\u2019s mailbox. And a refusal is the cheap branch. Success is the expensive one: an assessment is readable by anybody who obtains its code, the audit field carries the address of whoever wrote the verdict, and the roles collection becomes a roster of named public servants. Today those are personal addresses on an admitted prototype. Commitment: medium, and it is the request that puts the question of using Firestore at all onto somebody\u2019s desk at a time of their choosing.' },
          ...
          { t: 'Moving the store off Firestore', status: 'later', tier: 'level2',
            why: 'Commitment high, and it buys less than it looks. Firestore is named in two of the thirty-three modules and about 9,800 of 11,500 lines never learn where the store is, but the seam only covers the submitter document: roles, withdrawal and the code lookup call the store module directly from nine places in five files, so those get rewritten before a second store can exist at all. Three to five weeks for a destination somebody has named, and no estimate at all for one nobody has. What it does not buy is the listing problem: a rule cannot treat a value the client is holding as a credential, and that follows the identity rather than the store, so a server in front inherits it unchanged. What it would cost is enforcement. Google refuses a bad request today whether the page is honest or not; with a server in front that is our bug, our uptime and our thing to keep running after the work term ends.' },
        ] },
