...
export const quick = {
  ...
  items: [
    ...
    { t: 'Sign in, and a store with rules', status: 'next',
      why: 'deploy/firestore.rules is written: a Canadian region, nothing publicly readable, roles an admin grants, admin-only delete. Six one-time steps in a browser, and only you can do them because the account is yours. Two limits found before building it: sign-in has to be a Google account rather than an emailed link, because the free plan sends five of those a day for the whole project, and attachments cannot go in the store at all.' },
