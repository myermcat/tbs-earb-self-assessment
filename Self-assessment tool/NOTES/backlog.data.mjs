...
export const quick = {
  ...
  items: [
    ...
    { t: 'Teammates and assessors on an assessment', status: 'next',
      why: 'A mockup, working the way sharing a Google Doc works: enter an address, see who the assessment is shared with, remove somebody in two steps. It records the addresses on the record so the data survives into the real version, and it says plainly that no email is sent and no access is granted yet. The real version needs a rules change, which only you can publish.' },
